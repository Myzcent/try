...
-- Create animal vendor NPC (FIXED VERSION)
function CreateAnimalVendorNPC()
    ...
    CreateThread(function()
        ...
        -- Create NPC (removed -1.0 Z offset that was causing underground spawning)
        ...
        if Config.Debug then
            print('^2[Animal Farming]^0 Created animal vendor NPC at: ' .. vendor.coords.x .. ', ' .. vendor.coords.y .. ', ' .. vendor.coords.z)
        end
    end)
end
...
RegisterNetEvent('animal_farming:client:spawnAnimal', function(animalInfo)
    ...
    CreateThread(function()
        ...
        local modelHash = GetHashKey(animalConfig.model)
        ...
        if not HasModelLoaded(modelHash) then
            print('^1[Animal Farming]^0 Failed to load animal model: ' .. animalConfig.model)
            return
        end
        
        -- Spawn animal
        local animal = CreatePed(28, modelHash, coords.x, coords.y, coords.z, 0.0, true, false)
        
        if not DoesEntityExist(animal) then
            print('^1[Animal Farming]^0 Failed to create animal: ' .. animalId)
            SetModelAsNoLongerNeeded(modelHash)
            return
        end
        ...
        if Config.Debug then
            print('^2[Animal Farming]^0 Spawned animal: ' .. animalId .. ' (' .. animalType .. ') at: ' .. coords.x .. ', ' .. coords.y .. ', ' .. coords.z)
        end
    end)
end)
